...
class Order_admin_model extends CI_Model
{
    ...
    //delete bank transfer
    public function get_notification($str)
    {
        $this->db->where('class', $str, 'before');
    ...
    //delete bank transfer
    public function get_total_notification($str)
    {
        $this->db->select('admin_notifications.*, SUM(IF(notification = 0, 0, 1)) AS cnt');
        $this->db->where('SUBSTR(class, 1, 4) =', $str);
        $query = $this->db->get('admin_notifications');
        $row = $query->row();
        return $row->cnt;
    }
    ...
    //delete bank transfer
    public function show_notifications($str)
    {
        // $this->db->where('class')
        // $this->db->set('notification', 'notification+1', false);
        // $this->db->where('class', $str);
        // $this->db->update('admin_notifications');

        $data = array(
            'notification' => 0
        );
        $this->db->where('class', $str);
        return $this->db->update('admin_notifications', $data);
    }
}