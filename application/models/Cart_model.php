    ...
    public function calculate_cart_total()
    {
        ...
        if (!empty($cart)) {
            foreach ($cart as $item) {
                ...
                $top_parent_category_id = top_parent_category_id($product->category_id);
                $form_settings = $this->settings_model->get_form_settings($top_parent_category_id);
